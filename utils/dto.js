...
import { getMosaicRelativeAmount, getMosaicsWithRelativeAmounts, getNativeMosaicAmount } from './mosaic';
...
export const addressFromDTO = (address) =>
...
export const baseTransactionFromDTO = (transaction, config) => {
    ...
    return {
        ...
        id: transaction?.id,
        fee: getMosaicRelativeAmount(transaction.fee.value.toString(), config.networkProperties.networkCurrency.divisibility),
...
export const aggregateTransactionFromDTO = (transaction, config) => {
    const baseTransaction = baseTransactionFromDTO(transaction, config);
    const innerTransactions = transaction.transactions.map((innerTransaction) => transactionFromDTO(innerTransaction, config));
    ...
    // if (transaction.type.value === TransactionType.AGGREGATE_BONDED) {
...
export const transferTransactionFromDTO = (transaction, config) => {
    ...
    const messagePayload = transaction.message ? Buffer.from(transaction.message.subarray(1)).toString() : null;
