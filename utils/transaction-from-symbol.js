import { AddressRestrictionFlagMessage, AliasActionMessage, LinkActionMessage, LockHashAlgorithmMessage, Message, MosaicRestrictionFlagMessage, MosaicRestrictionTypeMessage, MosaicSupplyChangeActionMessage, NamespaceRegistrationTypeMessage, OperationRestrictionFlagMessage, TransactionType } from '@/constants';
...
export const aggregateTransactionFromSymbol = (transaction, config) => {
    const baseTransaction = baseTransactionFromSymbol(transaction, config);
    const innerTransactions = transaction.transactions.map((innerTransaction) => transactionFromSymbol(innerTransaction, config));
...
export const namespaceRegistrationTransactionFromSymbol = (transaction, config) => {
    ...
    return {
        ...
        duration: !!transaction.duration ? Number(transaction.duration) : Message.UNLIMITED,
    };
};
...
export const namespaceMetadataTransactionFromSymbol = (transaction, config) => {
    ...
    return {
        ...baseTransaction,
        scopedMetadataKey: transaction.scopedMetadataKey.toString(16),
        metadataValue: mapMetadataValue(transaction.value),
        valueSizeDelta: transaction.valueSizeDelta,
